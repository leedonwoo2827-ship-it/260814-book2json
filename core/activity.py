...
이 앱은 단계가 여덟 개뿐이지만 문제는 같다 — 각 단계는 자기 상태만 알고,
산출물은 폴더 깊이 흩어져 있다.
...
# 산출물 — 무엇을 만든 것인지 사람 말로. 이게 "원고를 낸 건지 프롬프트를 낸 건지"
...
def build(pid: int, slug: str, project: Dict[str, Any], *,
    ...
    # ── 만든 것 ────────────────────────────────────────────────────────────
    # ★ 여기가 핵심이다. "원고를 냈나 프롬프트를 냈나" 는 단계 목록이 아니라
    #   **파일이 언제 생겼나**로만 확실히 알 수 있다. 단계는 성공했는데 파일이
    #   안 나온 경우도 있어서(중간에 껐다든지), 파일 쪽을 따로 본다.
    dist = root / ws.STEPS["export"][0]
    if dist.is_dir():
        for f in sorted(dist.iterdir()):
            if not f.is_file():
                continue
            hit = next((d for d in DIST if f.name.endswith(d[0])), None)
            if not hit:
                continue
            at = _when(f)
            if at:
    ...
    # 아직 한 번도 안 돈 단계는 **숫자만** 센다. 원고 하나에 여덟 단계가 다
    # 필요한 경우는 거의 없어서(그림이 없는 원고는 b4 가 영영 안 돈다), 그것들이
...
def _stale_by_time(done: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    ★ 「낡음」 의 뜻을 정확히 이렇게 둔다(2026-08-14 지시):
        ...
        내보낸 것보다 **앞선 것**이, 지금 **산출물**보다 **나중에** 고쳐졌다.

      즉 목차를 고친 뒤 아직 조립을 안 했으면 5번이 낡았고, 조립한 뒤 아직 안
      내보냈으면 6번이 낡았다. 그게 전부다.

    ★ 예전에는 스테이지의 입력 해시가 어긋났는지로 판정했다. 그건 **사람이 확인할
      수 없는 기준**이다 — 방금 끝낸 단계가 자기 결과를 project.json 에 되써 넣어
      스스로를 낡게 만드는 일이 실제로 있었다. 시각 비교는 눈으로 검증된다 —
      "아까 목차를 고쳤으니 다시 조립해야지" 가 그대로 읽힌다.
...
def _stage_note(key: str, data: Any) -> str:
    ...
    if not isinstance(data, dict):
        return ""
    if key == "b1-pdf":
        return f"{data.get('pages') or 0}쪽 · {data.get('chars') or 0:,}자"
    if key == "b2-outline":
        n, g = len(data.get("slides") or []), len(data.get("groups") or [])
        return f"{n}장" + (f" · 묶음 {g}개" if g else "")
    if key == "b3-write":
        n = len(data.get("slides") or {})
        lines = data.get("lines") or 0
        return f"{n}장 · 줄 {lines}개" if n else ""
    if key == "b4-figure":
