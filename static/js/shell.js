 ...
 * 진행 상태와 원고 편집기의 미저장 텍스트가 살아 있다.
 *
 *   layer "base"  → #view 를 갈아치운다  (/outline /draft /image /export)
...
const routes = [
  ...
  /* ★ 목차가 맨 앞이다 — h3 하나가 슬라이드 한 장이므로, 여기서 정한 장 수가 곧
...
/* ★ 쇼케이스에 있던 「유형 메뉴」(슬라이드 텍스트 / +이미지 / +영상)는 없다.
 *   저기서 유형은 **그림이 어디서 오는가**였고 만드는 비용이 거기서 갈렸다.
 *   여기 원고는 전부 글이라 갈릴 축이 없다 — 한 장은 한 장이다.
 *   그 자리에 무엇을 억지로 채우지 않았다. 늘 비어 있는 칸은 눈이 배운다.
 */
...
async function refreshRail() {
  ...
  if (token !== railToken) return;
  $("#su-name").textContent = proj ? (proj.title || proj.slug) : "원고를 고르세요";
  $("#su-team").textContent = proj ? `${proj.slide_count || 0}장` : "—";
  $("#su-avatar").textContent = proj ? (proj.title || proj.slug).slice(0, 2) : "책";
}
